...
WHERE  EXISTS (
        ...
        FROM   (
         ...
         WHERE  f.Job = hist3.jh_job_id AND
                f.Server = hist3.jh_server
   )
AND datetime(hist3.jh_date) > datetime('now', '-1.04 day', 'localtime')
AND jh_step_id <> 0
AND jh_status = 0
GROUP  BY hist3.jh_server, job.sj_name, hist3.jh_step_name, hist3.jh_step_id